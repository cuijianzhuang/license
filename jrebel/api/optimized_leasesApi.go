...
// OptimizedLeasesController 优化版JRebel控制器
...
// 全局实例
var (
	optimizedController     *OptimizedLeasesController
	optimizedControllerOnce sync.Once
)

// NewOptimizedLeasesController 创建优化版控制器实例
func NewOptimizedLeasesController() (*OptimizedLeasesController, error) {
	var initError error
	optimizedControllerOnce.Do(func() {
	...
	return optimizedController, nil
}

// initPrivateKey 初始化RSA私钥（一次性解析）
func (oc *OptimizedLeasesController) initPrivateKey() error {
...
func (oc *OptimizedLeasesController) initObjectPools() {
	// 字符串构建器池
	...
	// VO对象池
	oc.leasesHandlerVOPool = sync.Pool{
		New: func() interface{} {
			return &vo.LeasesHandlerVO{}
		},
	}

	oc.leasesOneVOPool = sync.Pool{
		New: func() interface{} {
			return &vo.LeasesOneHandlerVO{}
		},
	}

	oc.validateVOPool = sync.Pool{
		New: func() interface{} {
...
func (oc *OptimizedLeasesController) startCleanupTask() {
...
func (oc *OptimizedLeasesController) cleanupExpiredCache() {
	now := time.Now()
	cleanedCount := 0

	oc.signatureCache.cache.Range(func(key, value interface{}) bool {
		entry := value.(*SignatureCacheEntry)
		if now.Sub(entry.timestamp) > signatureCacheTTL {
			oc.signatureCache.cache.Delete(key)
			atomic.AddInt64(&oc.signatureCache.entries, -1)
			cleanedCount++
		}
		return true
	})
	...
	// 如果缓存过大，删除最旧的条目
	if atomic.LoadInt64(&oc.signatureCache.entries) > signatureCacheMaxSize {
		oc.evictOldestEntries(signatureCacheMaxSize / 4) // 删除1/4的条目
	}

	oc.mu.Lock()
	oc.stats.LastCleanup = now
	oc.stats.CurrentCacheSize = atomic.LoadInt64(&oc.signatureCache.entries)
	oc.mu.Unlock()
}

// evictOldestEntries 淘汰最旧的缓存条目 (优化版排序算法)
func (oc *OptimizedLeasesController) evictOldestEntries(count int64) {
	type keyTime struct {
		key  interface{}
		time time.Time
	}
	...
	for i := int64(0); i < deleteCount; i++ {
		oc.signatureCache.cache.Delete(entries[i].key)
		atomic.AddInt64(&oc.signatureCache.entries, -1)
	}
}

// optimizedSign 优化版签名函数
func (oc *OptimizedLeasesController) optimizedSign(clientRandomness, guid string, offline bool, validFrom, validUntil int64) string {
	start := time.Now()
	...
	// 检查缓存
	if cachedEntry, found := oc.signatureCache.cache.Load(cacheKey); found {
		entry := cachedEntry.(*SignatureCacheEntry)
		if time.Since(entry.timestamp) < signatureCacheTTL {
			atomic.AddInt64(&oc.stats.CacheHits, 1)
			return entry.signature
		}
		// 过期条目，删除
		oc.signatureCache.cache.Delete(cacheKey)
		atomic.AddInt64(&oc.signatureCache.entries, -1)
	}

	// 缓存未命中，计算签名
	atomic.AddInt64(&oc.stats.CacheMisses, 1)
	...
	// 使用对象池获取哈希器
	hasher := oc.sha1HasherPool.Get().(hash.Hash)
	defer func() {
		hasher.Reset()
		oc.sha1HasherPool.Put(hasher)
	}()
	...
	// RSA签名（使用预解析的私钥）
	signature, err := rsa.SignPKCS1v15(rand.Reader, oc.privateKey, crypto.SHA1, hashed)
	if err != nil {
		return ""
	}
	...
	// 添加到缓存
	if atomic.LoadInt64(&oc.signatureCache.entries) < signatureCacheMaxSize {
		oc.signatureCache.cache.Store(cacheKey, &SignatureCacheEntry{
	...
	atomic.AddInt64(&oc.stats.TotalSignTime, signTime)
	atomic.AddInt64(&oc.stats.TotalRequests, 1)

	return signatureStr
}

// OptimizedLeasesHandler 优化版leases处理器
func (oc *OptimizedLeasesController) OptimizedLeasesHandler(c *gin.Context) {
	...
	signature := oc.optimizedSign(clientRandomness, guid, offline, validFrom, validUntil)

	// 从对象池获取VO对象
	leasesHandlerVO := oc.leasesHandlerVOPool.Get().(*vo.LeasesHandlerVO)
	defer oc.leasesHandlerVOPool.Put(leasesHandlerVO)
	...
	*leasesHandlerVO = vo.LeasesHandlerVO{
	...
	c.Header("X-Cache", fmt.Sprintf("hits:%d,misses:%d", 
		atomic.LoadInt64(&oc.stats.CacheHits), 
		atomic.LoadInt64(&oc.stats.CacheMisses)))

	c.JSON(http.StatusOK, leasesHandlerVO)
}

// OptimizedLeases1Handler 优化版leases/1处理器
func (oc *OptimizedLeasesController) OptimizedLeases1Handler(c *gin.Context) {
	...
	leasesOneVO := oc.leasesOneVOPool.Get().(*vo.LeasesOneHandlerVO)
	defer oc.leasesOneVOPool.Put(leasesOneVO)
...
// OptimizedValidateHandler 优化版validate处理器
func (oc *OptimizedLeasesController) OptimizedValidateHandler(c *gin.Context) {
	clientRandomness := c.PostForm("randomness")
	guid := c.PostForm("guid")
	offline, _ := strconv.ParseBool(c.PostForm("offline"))
	clientTime, _ := strconv.ParseInt(c.PostForm("clientTime"), 10, 64)
	...
	validateVO := oc.validateVOPool.Get().(*vo.ValidateHandlerVO)
	defer oc.validateVOPool.Put(validateVO)
...
func (oc *OptimizedLeasesController) GetPerformanceStats(c *gin.Context) {
	oc.mu.RLock()
	defer oc.mu.RUnlock()

	stats := *oc.stats
	stats.CurrentCacheSize = atomic.LoadInt64(&oc.signatureCache.entries)
	...
	if totalRequests > 0 {
		stats.AverageSignTime = atomic.LoadInt64(&oc.stats.TotalSignTime) / totalRequests
		
		cacheHits := atomic.LoadInt64(&oc.stats.CacheHits)
		cacheMisses := atomic.LoadInt64(&oc.stats.CacheMisses)
...
func (oc *OptimizedLeasesController) calculateLoadLevel(stats PerformanceStats) string {
	// 基于内存使用率计算负载级别
...
// HealthCheck 健康检查端点
func (oc *OptimizedLeasesController) HealthCheck(c *gin.Context) {
	...
	health := gin.H{
		...
		"cache_size": atomic.LoadInt64(&oc.signatureCache.entries),
...
// ClearCache 清空缓存
func (oc *OptimizedLeasesController) ClearCache(c *gin.Context) {
	cleared := int64(0)
	oc.signatureCache.cache.Range(func(key, value interface{}) bool {
		oc.signatureCache.cache.Delete(key)
		cleared++
	...
	atomic.StoreInt64(&oc.signatureCache.entries, 0)
	
	// 重置统计信息
	atomic.StoreInt64(&oc.stats.CacheHits, 0)
	atomic.StoreInt64(&oc.stats.CacheMisses, 0)
	atomic.StoreInt64(&oc.stats.TotalRequests, 0)
	atomic.StoreInt64(&oc.stats.TotalSignTime, 0)
...
// Shutdown 优雅关闭
func (oc *OptimizedLeasesController) Shutdown() {
...
func (oc *OptimizedLeasesController) RateLimitMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		select {
		case oc.requestLimiter <- struct{}{}:
			// 获取到令牌，继续处理
			defer func() { <-oc.requestLimiter }() // 处理完成后释放令牌
			c.Next()
		default:
			// 无法获取令牌，返回限流响应
			var memStats runtime.MemStats
...
// ForceGC 强制垃圾回收（调试用）
func (oc *OptimizedLeasesController) ForceGC(c *gin.Context) {
